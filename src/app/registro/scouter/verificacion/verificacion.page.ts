  ...
  async checkEmailVerification() {
    ...
    const interval = setInterval(async () => {
      ...
      if (user?.emailVerified) {
        ...
        this.router.navigate(['/registro/scouter/certificado']); // Redirigir a la siguiente página
      }
    }, 3000); // Verificar cada 3 segundos
  }

  continuar() {
    this.router.navigate(['/registro/scouter/certificado']); // Redirigir manualmente
  }
}